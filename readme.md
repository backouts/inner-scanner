...
이 문서는 inner 스캐너 프레임워크에서 사용하는 플러그인 모듈 구조와  
각 필드의 의미, 작성 규칙, 실행 흐름을 설명합니다.
...
MODULE은 모듈의 정적 정보와 옵션 정의를 담는 딕셔너리입니다.  
코어(scanner)는 이 정보를 사용해 모듈을 로딩·선택·검증합니다.
...
향후 run --category example 같은 배치 실행에 사용됨
...
코어는 이 정보를 기반으로 ctx["clients"]를 준비함
...
required=True인데 값이 없으면 코어가 실행 전에 차단해야 함

모듈 내부에서는 옵션 검증을 하지 않음
...
target = ctx["target"]
options = ctx["options"]
http = ctx["clients"]["http"]
```

4.2 run 함수의 책임 범위

run() 함수는 다음만 수행해야 합니다.
...
run()은 반드시 표준 Result 딕셔너리를 반환해야 합니다.
